...
<body>
    <div data-role="page" class="jqm-demos" data-title="ฐานข้อมูลสำหรับคริสตจักรนิมิตบ้านโฮ่ง">
        ...
        <?
            $week_id    = date('W');//34;//date('W');
            $year_id    = date('o');//2015;//date('o');
            $datetime   = date('Y-m-d H:i:s');//"2015-07-31 19:12:27";//date('Y-m-d H:i:s');
            $weekk      = $week_id.$year_id;//date('Wo');
        ?>
                ...
                <table data-role="table" id="table-custom-2" class="ui-body-d ui-shadow table-stripe ui-responsive" data-column-popup-theme="a">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            ?>
                            <th>ยอดยกมาจากอาทิตย์ที่ <?=$w?>/<?=$y?></th>
                            <th>
                                <div class="div2">
                                    <?
                                        $weeek      = $w.$y;
                        ...
                        <tr>
                            ...
                            <th>
                                ...
                                // select ข้อมูล username ว่าเคยมีการใช้งานไปแล้วหรือไม่
                                    $sqlC	= "SELECT * FROM balance where week=$weekk";
                                ...
                                // ถ้า username มีการใช้งนไปแล้ว จะให้แจ้งเตือนเพื่อให้เปลี่ยน username ที่ต้องการใช้
                                    ...
                                    {
                                        // ถ้ายังไม่เคยบันทึก
                                        $sql= "INSERT INTO `DBC`.`balance` (`balance_id`, `date`, `week`, `expenses_amount`, `donation_amount`, `balance`) VALUES ('', '$datetime', '$weekk', '$e', '$d', '$b')";
                                        mysql_query($sql) or die("error=$sql");
                                    }
                                    ?>
                            </th>
