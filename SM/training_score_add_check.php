...
// ตรวจสอบการกรอกข้อมูลที่ไม่ควรให้มีการเว้นว่าง
    if(empty($regis_id)||empty($training_unit)||empty($training_score)||empty($course_detail_id))
	{
...
// select ข้อมูล username ว่าเคยมีการใช้งานไปแล้วหรือไม่
	...
	if($numC != 0)
	{
		echo "<script>alert('ให้คะแนนแล้ว!!!');window.location='training_score_add.php?course_detail_id=$course_detail_id&training_unit=$training_unit';</script>";
		exit();
	}else
	{
        // ข้อมูล member
        $date = date("Y-m-d H:i:s");//วันที่ลงทะเบียน
        $sql= "insert into trainingscore values('$regis_id','$training_unit','$date','$cs','$training_score')";
